...
class VelPub(Node):
    ...
    def image_callback(self, msg: Image):
        ...
        if not self.white_found and abs(self.line_tracker.delta_white) > threshold_value:
            self.vel.linear_velocity = 0.0  # Stop the car when a white line is detected
            self.white_found = True
            self.get_logger().info('found whiteline')
            if self.stoptime is None and self.current_linear_x == 0:
                self.stoptime = dt.datetime.now()

        # Check if the car is stopped and has been stopped for more than 3 seconds
        if self.white_found and self.stoptime is not None:
            elapsed_time = (dt.datetime.now() - self.stoptime).total_seconds()
            if elapsed_time > 3.0:
        ...
        if not self.white_found and elapsed_time > 3.0:
            ...
            new_linear_velocity = 2.0 if (self.vel.angular_velocity >= 0.070) or (
                    self.vel.angular_velocity <= -0.070) else 4.5
